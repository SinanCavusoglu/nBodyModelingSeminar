...
def oscillation_experiment_overrides() -> list[dict[str, Any]]:
    ...
    exps.extend(
        [
            base_expansion_experiment(name="osc_radial_0p05", velocity_mode="radial", velocity_scale=0.05, softening=2.0, H0=0.01),
            base_expansion_experiment(name="osc_angular_0p05", velocity_mode="angular", velocity_scale=0.05, softening=2.0, H0=0.01),
            base_expansion_experiment(name="osc_radial_0p01", velocity_mode="radial", velocity_scale=0.01, softening=2.0, H0=0.01),
            base_expansion_experiment(name="osc_angular_0p01", velocity_mode="angular", velocity_scale=0.01, softening=2.0, H0=0.01),
            base_expansion_experiment(name="osc_none", velocity_mode="none", velocity_scale=0.0, softening=2.0, H0=0.01),
        ]
    )
    ...
    # Set 3: Does expansion/damping reduce oscillation? Include Sebastian's 2.7% economic growth idea.
    ...
    # Optional scalable final candidate with Barnes-Hut.
    exps.append(
        base_expansion_experiment(
...
def run_custom_experiments(experiments: list[dict[str, Any]], common_overrides: dict[str, Any]) -> list[dict[str, Any]]:
    outputs = []
...
def run_oscillation_experiments(common_overrides: dict[str, Any]) -> list[dict[str, Any]]:
    ensure_oscillation_velocity_inputs()
    common = dict(common_overrides)
    common["ANALYZE_OSCILLATION"] = True
    return run_custom_experiments(oscillation_experiment_overrides(), common)
...
def save_run_index(outputs: list[dict[str, Any]], output_path: Path, run_manifest: dict[str, Any] | None = None) -> None:
    ...
    should_analyze = any(str(row.get("experiment", "")).startswith("osc_") for row in rows)
